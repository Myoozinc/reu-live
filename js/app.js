...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const generateExecutivePDF = (meeting) => {
      ...
      <div class="pdf-header">
        <div class="pdf-brand" style="display: flex; align-items: center; gap: 12px;">
          ...
          <div>
            ...
            <p style="margin: 2px 0 0 0; font-size: 8.5pt; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">Acta & Informe Ejecutivo de Reunión</p>
          </div>
        </div>
    ...
    if (window.html2pdf) {
      const filename = `Reu.live-Acta-${meeting.id || Date.now()}.pdf`;
      const opt = {
        margin: [10, 10, 10, 10],
        filename: filename,
